        ...
        <motion.section
          ...
          {projects?.length > 0&& projects.map((project, index) => (
            <motion.div
              ...
              whileHover={{translateY: 'var(--y-hover,0)', x:'var(--x-hover,0)', transition:{when:'afterChildren'}}}
              className="min-h-80 max-w-ful sm:max-w-80 bg-white flex-col relative border-gray-900 border-[1px] cursor-pointer" key={index}>
              <div className="w-full h-3/5 relative0 overflow-hidden">
                <img src={project.img} alt='' className="h-full w-full object-scale-down hover:scale-105 bg-white " />
              </div>
              <div className="py-2 px-4" >
                <h3 className="text-md">{project.title}</h3>
                <p className="line-clamp-4 text-sm leading-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Beatae maxime ipsa aliquid, quas error quis qui libero accusantium reprehenderit eaque culpa quod facilis assumenda nesciunt ipsam et aut iure porro!</p>
              </div>
